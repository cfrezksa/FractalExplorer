...
float sampleSpace( vec3 vecPos)
{
    ...
    float tt = 0.55 + 0.1 * sin(0.2*time + rayOrigin.x + rayOrigin.z + vecPos.y * cos(vecPos.x + vecPos.z)); 
    
	orb = vec4(1000.0); 
	
	for( int i=0; i< iterations;i++ )
	{
		vecPos = -1.0 + 2.0 * fract(0.5 * vecPos + 0.5);
	...
	return 0.25*abs(vecPos.y)/scale;
}

float timeWarp() {
	return 1.6;//(1.1 + 0.5 * smoothstep( -0.3, 0.3, cos(time) ));
}
...
vec4 transformAlbedo() {
	...
	rgb = mix( rgb, color2, pow(clamp(1.0 - 2.0 * orb.z, 0.0, 1.0), 8.0));
...
vec3 shading() {
    ...
    // color
    vec3 col = brdf * albedo.rgb * exp(- lambda);
